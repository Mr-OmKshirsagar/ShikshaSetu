...
import { Save, UserRound, Briefcase, ShieldCheck } from "lucide-react";
...
export function AdminProfile() {
  ...
  // Sync when user object loads asynchronously
  useEffect(() => {
    if (!user) return;
  ...
  const inputCls = "w-full rounded-xl border border-slate-200 px-4 py-2.5 text-sm text-[#123057] focus:border-[#4b36a8] focus:outline-none transition-colors";
  ...
  return (
    <form onSubmit={handleSave} className="space-y-6 max-w-3xl mx-auto anim-page-enter">
      ...
      <div className="rounded-3xl border border-[#dfe7f0] bg-white p-6 shadow-sm space-y-4 anim-card-enter stagger-2">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 rounded-xl border border-slate-100 bg-slate-50 px-4 py-2.5 text-sm font-semibold text-[#4b36a8]">
              <ShieldCheck size={14} /> Full Organisational Intelligence Access
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="rounded-2xl border border-[#dfe7f0] bg-white p-6 shadow-sm space-y-4">
        <div className="flex items-center gap-2.5 pb-3 border-b border-slate-100">
          <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-purple-50">
            <Briefcase size={16} className="text-[#4b36a8]" />
          </div>
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <div>
            ...
            <input className={inputCls} value={designation} onChange={e => setDesignation(e.target.value)} placeholder="e.g. Director (Administration)" />
          </div>
          <div>
            <label className={labelCls}>Department / Ministry</label>
            <input className={inputCls} value={department} onChange={e => setDepartment(e.target.value)} placeholder="e.g. Department of Personnel and Training" />
          </div>
          <div className="sm:col-span-2">
            <label className={labelCls}>Organization / Office</label>
            <input className={inputCls} value={organization} onChange={e => setOrganization(e.target.value)} placeholder="e.g. DoPT, North Block, New Delhi" />
          </div>
        </div>
      </div>
      ...
      <div className="flex justify-end pb-8">
        <button
          type="submit"
          disabled={saving}
          className="inline-flex items-center gap-2 rounded-xl bg-[#4b36a8] px-8 py-3 text-sm font-bold text-white shadow hover:bg-[#3d2a9a] disabled:opacity-50 transition-all active:scale-95"
        >

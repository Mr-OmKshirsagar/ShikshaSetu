...
export function AdminUsers({ onNavigate }: AdminUsersProps) {
  ...
  return (
    <>
      <div className="space-y-8 anim-page-enter max-w-6xl mx-auto">
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between anim-fade-up">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-1">
              Active civil servants, curriculum trainers, and system administrators.
              Click any row to view an individual workforce profile.
            </p>
          </div>
          <button
            onClick={fetchUsers}
            className="flex items-center gap-1.5 rounded-xl border border-[#e0daef] bg-white px-4 py-2 text-xs font-semibold text-[#4b36a8] shadow-sm hover:bg-purple-50 transition-all"
          >
        ...
        <div className="flex flex-col gap-3 rounded-2xl border border-[#e0daef] bg-white p-4 sm:flex-row sm:items-center anim-card-enter stagger-1">
          ...
          <div className="flex items-center gap-2">
            <Filter size={15} className="text-slate-400" aria-hidden />
            <select
              value={selectedRole}
        ...
        <div className="rounded-3xl border border-[#e0daef] bg-white shadow-sm overflow-hidden anim-card-enter stagger-2">
          <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
            ...
            <span className="text-xs text-slate-400 font-medium">
              Click a row to view workforce profile
            </span>
          </div>
          ...
          {loading ? (
            <div className="space-y-2 p-6">
              {[1, 2, 3].map((n) => (
                <div key={n} className="h-12 rounded-2xl bg-slate-100 animate-pulse" />
              ))}
            </div>
          ) : filteredUsers.length === 0 ? (
            <div className="p-10 text-center">
              <Users size={28} className="mx-auto text-slate-300 mb-3" />
              <p className="text-sm font-bold text-slate-500">No users match your filter.</p>
            </div>
          ) : (
                ...
                <tbody className="divide-y divide-slate-100">
                  {filteredUsers.map((user, idx) => (
                    <tr
                      ...
                      tabIndex={0}
                      role="button"
                      aria-label={`View workforce profile for ${user.full_name}`}
                      className={`hover:bg-purple-50/60 cursor-pointer transition-colors focus:outline-none focus-visible:bg-purple-50 anim-card-enter stagger-${Math.min(idx + 1, 6)}`}
                    >
                      <td className="px-6 py-4">
                        <div className="font-bold text-[#123057]">{user.full_name}</div>
                        <div className="text-[11px] text-slate-400">
                          {user.email} &middot;{" "}
                          <span className="font-mono text-[10px]">{user.employee_id}</span>
                        </div>
                      </td>
                      <td className="px-6 py-4 font-semibold text-slate-600">{user.department}</td>
                      <td className="px-6 py-4">
      ...
      {/* Individual workforce profile panel */}
      {profileUser && (
        <AdminUserProfile
